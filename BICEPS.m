%{
this code will extract the neural and kinematic data from their raw formats, align them, and create a save for each. 
the muscle and joint data is taken from the sto files generated by moco during the simulation. In addition to this, the raw kinematic data is taken from the live recordings, and arithmatic is done for extra variables such as
acceleration, displacement (in the xyz direction individually), and displacement norm. additional muscle fibre data is avaliable but is not included in this code. it is commented out. movmean is used to smooth the muscle data
to make it more realistic visually. there are 40 timesteps per reach, and each timestep corresponds to 10ms.

IMPORTANT!!!
the neuropixel data has to be aligned to the kinematic data timepoints, and is done so using the idx values. we also take the full lag sections of the neural data. ie, if we want a minimum lag of -120 and a maximum lag of +15,
the saved neural data will encompass all neural data from -120 to +15, including the lenght of the reach. A total of 175 timepoints will be present per reach, and will have to be cut down accordingly to analyze. 
This is a timesaver instead of saving each lagged dataset separately. 

the neural data needed to be interpolated to get 40 timepoints per reach.
%}
